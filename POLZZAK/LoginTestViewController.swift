...
class LoginTestViewController: UIViewController {
    ...
    private func bind() {
            ...
            .sink { [weak self] _ in
                ...
                Task {
                    ...
                    case 400:
                        ...
                        self.username = needRegisterDTO.username
                        self.socialType = needRegisterDTO.socialType
                    default:
                        return
                    }
                }
        ...
        appleLoginButton.addGestureRecognizer(tapGesture)
        tapGesture.tapPublisher
            .sink { [weak self] _ in
                guard let self else { return }
                
                Task {
                    guard let (data, response) = try? await AuthAPI.appleLogin(appleLoginPresentationAnchorView: self), let httpResponse = response as? HTTPURLResponse else { return }
                    let statusCode = httpResponse.statusCode
        ...
        registerButton.tapPublisher
            .sink { [weak self] _ in
                guard let self else { return }
                guard let username = self.username, let socialType = self.socialType else { return }
