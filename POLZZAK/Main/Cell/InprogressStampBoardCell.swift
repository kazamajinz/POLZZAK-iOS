...
class InprogressStampBoardCell: UICollectionViewCell {
    ...
    private let requestGradationView: MessageView = {
        let label = MessageView(type: .request)
        label.setContentHuggingPriority(.defaultHigh, for: .vertical)
        return label
    }()
    
    private let completedGradationView: MessageView = {
        let label = MessageView(type: .completed)
        label.setContentHuggingPriority(.defaultHigh, for: .vertical)
        return label
    }()
...
extension InprogressStampBoardCell {
    ...
    private func setUI() {
        ...
        requestGradationView.snp.makeConstraints {
            $0.top.equalToSuperview()
            $0.centerX.equalToSuperview()
        }
        ...
        stampCompletedImageView.snp.makeConstraints {
            $0.top.equalTo(requestGradationView.snp.bottom).offset(6)
            $0.centerX.equalToSuperview()
